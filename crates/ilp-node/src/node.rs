...
impl InterledgerNode {
    ...
    where
        S: NodeStore<Account = Account>
            ...
            + 'static,
    {
        println!("[MY_LOG DEBUG] {}:{}", file!(), line!()); debug!(target: "interledger-node",
                        ...
                        .and_then(
                            move |new_level_input: bytes05::Bytes| {
                                ...
                                async move {
                                    ...
                                    println!("[MY_LOG DEBUG] {}:{}", file!(), line!()); debug!(target: "interledger-node", "Logging level adjusted to {}", new_level_str);
        ...
        println!("[MY_LOG INFO] {}:{}", file!(), line!()); info!(target: "interledger-node", "Interledger.rs node HTTP API listening on: {}", http_bind_address);
        ...
        println!("[MY_LOG INFO] {}:{}", file!(), line!()); info!(target: "interledger-node", "Settlement API listening on: {}", settlement_api_bind_address);
        ...
        if let Some(provider) = exchange_rate_provider {
            ...
            println!("[MY_LOG DEBUG] {}:{}", file!(), line!()); debug!(target: "interledger-node", "Not using exchange rate provider. Rates must be set via the HTTP API");
        }
