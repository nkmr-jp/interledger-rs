    ...
    A: Account + 'static,
{
    async fn handle_request(&mut self, request: IncomingRequest<A>) -> IlpResult {
        println!("[MY_LOG INSPECT] IncomingService.handle_request() request.prepare: {:?} {}:{} ",request.prepare, file!(), line!());
        self.inner_mut()
    ...
    A: Account + 'static,
{
    async fn send_request(&mut self, request: OutgoingRequest<A>) -> IlpResult {
        println!("[MY_LOG INSPECT] OutgoingService.send_request() request.prepare: {:?} {}:{} ",request.prepare, file!(), line!());
        self.inner_mut()
